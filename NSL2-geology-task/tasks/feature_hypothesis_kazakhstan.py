    ...
    def evaluate_saturation(self, generation_data: Any) -> SaturationOutcome:
        ...
        try:
            interweave_remaining = int(
                self._read_interweave_state(self._kg_dir / "teniz_basin").get(
    ...
    def rollover_knowledge_graph(
        ...
        *,
        ...
        with self._saturation_lock:
            ...
            epoch_episodes = all_episodes[epoch_start_count:]
            kg_active = self._kg_dir / "teniz_basin"
            store_active = self._store_dir / "teniz_basin"
            ...
            archive_dest = (
                ...
                / f"kazakhstan-teniz-basin-{stamp}-{reason_component}"
            )
            ...
            for src, name in (
                (store_active, "store_teniz_basin"),
                (kg_active, "knowledge_teniz_basin"),
            ):
                if src.exists():
            ...
            logger.info(
                "feature_hypothesis_kazakhstan: archived saturated KG to "
...
'''
    ...
    def _seed_phase_admission_ok(cls, kg_record: dict[str, Any], seed_phase: bool) -> bool:
        ...
        # gen3-kz deadlock (2026-07-03): a founder below the min fill-fraction floor
        # is not a viable cross-lift target — a 2-voxel blob admitted
        # first_layer_auto poisoned the cross-only pool (n_eff=0 for every later
        # candidate -> KG frozen at 1). Reject the LOW-but-nonzero founder here;
        # empty (fill 0) is already handled by the emptiness gate upstream in
        # ``check_guards``.
        from voxel_features.scoring import _SPATIAL_MIN_FOUNDER_FILL_FRACTION

        try:
            fill = float(kg_record.get("candidate_fill_fraction", 0.0) or 0.0)
        except (TypeError, ValueError):
            fill = 0.0
        if 0.0 < fill < _SPATIAL_MIN_FOUNDER_FILL_FRACTION:
    ...
    def _stamp_candidate_triviality(cls, kg_record: dict, *, values: Any) -> None:
        ...
        #     the gate before: declared_nothing required op_count == 0, and the seed
        #     floor's single_spatial_operation excludes array ops, so a single all-zero
        #     array op was admitted first_layer_auto (2026-06-26 gen-2 DFT run).
