...
def _task(tmp_path: Path, **config: object) -> FeatureHypothesisKazakhstanTask:
...
def test_evaluate_saturation_detects_interweave_plateau(tmp_path: Path) -> None:
    task = _task(
        tmp_path,
        saturation_enabled=True,
    ...
    kg_dir = tmp_path / "kg_root" / "teniz_basin"
...
def test_rollover_preserves_training_rows_and_archives_kg(tmp_path: Path) -> None:
    task = _task(tmp_path, saturation_enabled=True)
    kg_dir = tmp_path / "kg_root" / "teniz_basin"
    store_dir = tmp_path / "store_root" / "teniz_basin"
    ...
    assert (archives[0] / "knowledge_teniz_basin" / "experiments.jsonl").exists()
    assert (archives[0] / "store_teniz_basin" / "admitted" / "layers" / "layer.bin").exists()


def test_epoch_window_uses_append_count_not_episode_index(tmp_path: Path) -> None:
    task = _task(
        tmp_path,
        saturation_enabled=True,
        saturation_min_episodes=3,
        saturation_success_rate_floor=0.50,
        saturation_max_rollovers=99,
    )
    data = _generation([_episode(i + 100, success=False) for i in range(50)])
    ...
    data.add_episode(_episode(200, success=False))
    data.add_episode(_episode(201, success=False))
...
def test_unproductive_guard_uses_kg_admissions_not_training_rows(tmp_path: Path) -> None:
    task = _task(
        tmp_path,
        saturation_enabled=True,
        saturation_min_episodes=10,
        saturation_success_rate_floor=0.50,
        saturation_max_rollovers=1,
        saturation_epoch_progress_admissions=1,
    )
    data = _generation(
        [_episode(0, success=True, row_count=5)]
        + [_episode(i, success=False) for i in range(1, 10)]
    )

    outcome = task.evaluate_saturation(data)

    assert data.training_row_count == 5
    assert outcome.decision is SaturationDecision.STOP
    assert outcome.reason == "saturation_exhausted"


def test_unproductive_guard_resets_on_kg_admission(tmp_path: Path) -> None:
    task = _task(
        tmp_path,
        saturation_enabled=True,
        saturation_min_episodes=10,
    ...
    data = _generation(
        [_episode(0, success=True, row_count=5, admitted=True)]
        + [_episode(i, success=False) for i in range(1, 10)]
    )
...
def test_rollover_prunes_archives_to_retention_limit(tmp_path: Path) -> None:
    task = _task(
        tmp_path,
        saturation_enabled=True,
        saturation_archive_keep_last=2,
    )
    kg_dir = tmp_path / "kg_root" / "teniz_basin"
    data = _generation([_episode(0, success=False)])

    for idx in range(3):
        (kg_dir / f"marker-{idx}.jsonl").write_text("{}\n", encoding="utf-8")
        task.rollover_knowledge_graph(
            f"C1 epoch {idx}",
            data,
            rolled_at=f"2026-07-07T00:00:0{idx}",
        )

    archives = sorted(path.name for path in task._saturation_archive_dir.iterdir())

    assert len(archives) == 2
    assert all("00-00-00" not in name for name in archives)
